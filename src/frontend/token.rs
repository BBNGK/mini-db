...
impl Tokenizer {
	...
	fn new (input: String) -> Self {
		...
		Tokenizer{ input, position: 0, keyword, operation }
	}
	fn get_next_token (&mut self) -> Option<Token> {
		...
		if current_character.is_alphabetic() {
			...
			self.position += 1;
			return Some(Token::new(TokenType::Unknown, current_character.to_string()))
		}
	}
...
>>>>>>> 2b92368 (Tokenizer for SQL keywords):src/token.rs
